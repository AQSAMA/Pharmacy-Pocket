...
export default function HomeScreen() {
  ...
  useEffect(() => { list.current?.scrollToOffset({ offset: 0, animated: false }); }, [category, deferredQuery, favoritesOnly]);
  ...
  }, [items, category, deferredQuery, favoritesOnly]);
  ...
  return <View style={[styles.screen, { paddingTop: insets.top }]}>
    <FlatList
      ...
      ListHeaderComponent={<View style={{ paddingTop: 9, paddingHorizontal: 16, gap: 12 }}>
    ...
    <View style={{ paddingBottom: Math.max(insets.bottom, 9), paddingTop: 9, backgroundColor: '#ffffff', borderTopWidth: 1, borderTopColor: '#dce5e1', gap: 7 }}>
      <ScrollView horizontal style={{ flexGrow: 0, flexShrink: 0 }} showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 10, gap: 6 }}>
        {categories.map((item) => <Pressable key={item.id} onPress={() => setCategory(item.id)} style={{ minHeight: 42, justifyContent: 'center', paddingHorizontal: 15, borderRadius: 11, backgroundColor: category === item.id ? '#103e3b' : '#eff4f1' }}><Text style={{ color: category === item.id ? '#ffffff' : '#536c62', fontSize: 16, writingDirection: 'rtl' }}>{item.arabic}</Text></Pressable>)}
      </ScrollView>
