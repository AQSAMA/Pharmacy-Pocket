...
export default function CustomerPriceScreen() {
  ...
  const { items, currency, largeText } = useMedicines();
  ...
  return <ScrollView contentInsetAdjustmentBehavior="automatic" contentContainerStyle={{ padding: 18, paddingBottom: 34, gap: 14 }}>
    ...
    <View style={{ backgroundColor: '#ffffff', borderRadius: 18, padding: 18, gap: 12 }}>
      {[
        ['Category', categoryById(item.category).label],
        ['Subcategory', subcategoryLabel(item.subcategory)],
        ['Date added', formatAddedDate(item.createdAt)],
      ].map(([label, value]) => <View key={label} style={{ gap: 4 }}>
        <Text style={{ color: '#60766d', fontSize: largeText ? 16 : 13 }}>{label}</Text>
        <Text selectable style={{ color: '#315247', fontSize: largeText ? 22 : 16, fontWeight: '600' }}>{value}</Text>
      </View>)}
      <Text style={{ color: '#60766d', fontSize: 12, lineHeight: 18 }}>Older records may show the date they were imported or migrated.</Text>
    </View>
