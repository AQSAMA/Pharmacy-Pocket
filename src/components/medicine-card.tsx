...
type Props = { item: Medicine; large: boolean; currency: string; first: boolean; last: boolean; onFavorite(): void };

function MedicineCardComponent({ item, large, currency, first, last, onFavorite }: Props) {
  const rtl = hasArabic(item.name);
  return (
    <Pressable accessibilityRole="button" accessibilityLabel={`${item.name}, official price ${item.official} ${currency}`} onPress={() => router.push({ pathname: '/medicine/[id]', params: { id: item.id } })} style={({ pressed }) => [styles.card, first && styles.firstCard, last && styles.lastCard, pressed && styles.pressedCard]}>
